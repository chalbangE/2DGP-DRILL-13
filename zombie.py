...
class Zombie:
    ...
    def Ball_count_comparison_boy_big(self):
        if self.ball_count >= play_mode.boy.ball_count:
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL

    def Ball_count_comparison_boy_small(self):
        if self.ball_count < play_mode.boy.ball_count:
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL
    ...
    def Running_away_to_boy(self, r=0.5):
        self.state = 'Walk'
    ...
    def build_behavior_tree(self):
        ...
        c2 = Condition('소년의 공이 더 적은가?', self.Ball_count_comparison_boy_big)
        c3 = Condition('소년의 공이 더 많은가?', self.Ball_count_comparison_boy_small)
        a4 = Action('접근', self.move_to_boy)
        a5 = Action('도망', self.Running_away_to_boy)
        SEQ_chase_boy = Sequence('소년을 추적', c1, c2, a4)
        SEQ_runnig_away_boy = Sequence('소년에게서 도망', c1, c3, a5)
        root = SEL_chase_or_flee = Selector('추적 또는 도망 또는 배회', SEQ_chase_boy, SEQ_runnig_away_boy, SEQ_wander)
